...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_metric", type=str, nargs="*", help="xgboost eval metric", required=True
    )
    parser.add_argument(
        "--objective", type=str, nargs="*", help="xgboost objective", required=True
    )
    ...
    xgboost_eval_metric_list = args.eval_metric
    xgboost_objective_list = args.objective
    ...
    if args.train:
        for sig_tree, sig_label in zip(my_sig_trees, my_sig_labels):
            ...
            for category, data in zip(categories, full_data):
                ...
                for xgboost_eval_metric in xgboost_eval_metric_list:
                    for xgboost_objective in xgboost_objective_list:
                        category_dir = f"{results_dir}/cat_{category}/{xgboost_eval_metric}_{xgboost_objective}"
                        # make directory
                        ...
                        print(f"Training objective {xgboost_objective} complete!")
                    print(f"Training eval metric {xgboost_eval_metric} complete!")
